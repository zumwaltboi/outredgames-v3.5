  ...
  .then(data => {
    ...
    function renderGrid(items) {
      ...
      items.forEach(item => {
        ...
        game.innerHTML = `<a href="#"onclick="localStorage.setItem('currentgame', '${item.link}'); localStorage.setItem('currenttitle', '${item.title}'); localStorage.setItem('currentdescription', '${item.description}'); window.location.href = '/g/load.html';"><img src="${item.image}" loading="lazy"; alt="${item.title}"><h2>${item.title}</h2></a>`;
